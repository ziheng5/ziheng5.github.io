...
> ✍ 做题小结论：
> - 如果 $X$ 的分布函数 $F(x)$ 是连续函数，则有 $Y=F(X) ～ U(0, 1)$【余丙森强化`例 2.14`】
> - 如果遇到 $Y = F(X)$ 这种分布套分布的问题，可以尝试将 $F(X)$ 图像画出来分类讨论，然后 $G(y) = P\{Y \le y\} = P\{F(X) \le y\}$【880-17-综合-选择-4】
...
1. 概率密度
   ...
   - （2）性质（会出选择题）
     - a. $f(x) \ge 0$
     - b. $\int^{+\infty}_{\infty} f(x) dx = 1$

> 💡 关于概率密度性质的一道例题：设 $f(x)$ 为随机变量 $X$ 的概率密度，则下列选项可以作为某一随机变量的概率密度的是（）
> - A. $f(1-x)$（✅，因为 $\int^{+\infty}_{-\infty}f(1-x) dx = -\int^{+\infty}_{-\infty}f(1-x) d(1-x) = \int^{+\infty}_{-\infty}f(t) dt = 1$）
> - B. $f(\dfrac{x}{2})$（❌，因为 $\int^{+\infty}_{-\infty} f(\dfrac{x}{2}) dx = 2\int^{+\infty}_{-\infty} f(\dfrac{x}{2}) d\dfrac{x}{2} = 2 \ne 1$）
> - C. $f(x^2)$（❌，积分不为 1）
> - D. $f^2(x)$（❌，积分不为 1）
...
3. 指数分布
   - （1）$X～E(\lambda)$，其中 $\lambda \ge 0$
   - （2）概率密度：$f(x) = \begin{cases} \lambda e^{-\lambda x}, & x>0 \\ 0, & otherwise\end{cases}$
   - （3）分布函数：$F(X) = \begin{cases} 0, & x\le 0 \\ 1-e^{-\lambda x}, & x>0\end{cases}$
   - （4）指数分布也具有**无记忆性**
   - （5）指数分布的一种变种：$f(x) = \begin{cases} \lambda e^{-\lambda(x - \theta)}, & x \ge \theta \\ 0, & x < \theta \end{cases}$
   - （6）若 $X_1～E(\lambda_1), X_2～E(\lambda_2)$，则 $Y = \min{X_1, X_2} ～ E(\lambda_1 + \lambda_2)$。该结论证明很简单，相关题目见【880-19-综合-解答-3、4】
...
> ⚠️ 本章涉及易错点：分类讨论、区间判定、二重积分，计算时极其容易在取值范围上出错，一定要谨慎分类讨论，可画图辅助判断！

> 💡 本章重点习题
> - 【880-18-基础-选择-1、3】
> - 【880-18-基础-解答-2、3、5、6、9】
> - 【880-18-综合-选择-2】
> - 【880-18-综合-填空-1】
> - 【880-18-综合-解答-5、7、8】
...
> 💡 设二维随机变量 $(X_1, X_2)$ 的概率密度为 $f_1(x_1, x_2), Y_1 = 2X_1, Y_2 = 3X_2$，则 $(Y_1, Y_2)$ 的概率密度 $f_2(y_1, y_2)$ 计算方式：
> - ![pos_1](/images/mathematic/pos_1.png)
...
> ⚠️ 二维 $\textcolor{red}{离散}$ 随机变量 $(U, V)$ 的联合概率分布最好写成表格的形式

> 💡 假设 $F(X)$ 式子比较复杂，要计算 $P\{F(X) \le u\}$ 时，可以这样计算：
> - $P\{F(X) \le u\} = P\{X \le F^{-1}(u)\} = \int^{F^{-1}(u)}_{-\infty} f(x) dx = \int^{F^{-1}(u)}_{-\infty} dF(x) = u - F(-\infty)$（神乎其技）
...
9. $\max\{...\}$ 与 $\min\{...\}$
    ...
    - （3）$F(\max\{X_1, X_2, \cdots, X_n\}) = F(X_1)F(X_2)\cdots F(X_n)$。若 $X_1, X_2, \cdots, X_n$ 独立同分布，则有 $F(\max\{X_1, X_2, \cdots, X_n\}) = F(X)^{n} \Rightarrow f(\max\{X_1, X_2, \cdots, X_n\}) = n F(X)^{n-1} \cdot f(x)$【880-19-综合-解答-1】
    - （4）$F(\min\{X_1, X_2, \cdots, X_n\}) = 1 - [1 - F(X_1)][1 - F(X_2)] \cdots [1 - F(X_n)]$。若 $X_1, X_2, \cdots, X_n$ 独立同分布，则有 $F(\min\{X_1, X_2, \cdots, X_n\}) = 1 - [1 - F(X)]^{n} \Rightarrow f(\min\{X_1, X_2, \cdots, X_n\}) = n [1 - F(X)]^{n-1} \cdot f(x)$
...
6. 不相关与独立
   - （1）用韦恩图来表示的话，`独立` 是被包含在 `不相关` 里的，即独立一定不相关，但不相关不代表独立（逆否命题：若 $X$ 与 $Y$ 相关，则 $X, Y$ 不独立）
...
> 💡 若题目给出“样本观察值为 $x_1$、$x_2$、...、$x_n$”（哪怕只有一个样本 $x_1$），那么
> - 矩估计中样本均值就是这几个样本的平均值
...
> ⚠️ 若最大似然估计法求解据估计量 $\hat{\theta}$ 的过程中，求导之后发现函数为**单调递增**（或**单调递减**），那么 $\hat{\theta}$ 直接取得边界值，比如边界值为 $x\ge \theta (x\le \theta)$，那么据估计量 $\hat{\theta}$ 等于 $\min\{X_1, X_2, ..., X_n\} (\max\{X_1, X_2, ..., X_n\})$
...
> - 但样本标准差 $S$ 不是总体标准差 $\sigma$ 的无偏估计量！
> - **未修正的样本标准差**是总体标准差 $\sigma$ 的**最大似然估计量**
> - 样本标准差 $S$ 不是总体标准差 $\sigma$ 的最大似然估计
> - 样本标准差 $S$ 是总体标准差 $\sigma$ 的**相合估计量**
...
6. **犯两类错误的概率**
   - （1）第一类错误：$\alpha = P\{$拒绝$H_0|H_0$为真$\}$
   - （2）第二类错误：$\alpha = P\{$接受$H_0|H_0$为假$\}$


> ⚠️ 易错点：假设题目告诉你原假设 $H_0$、备择假设 $H_1$，拒绝域 $W = \{\overline{X} > 3\}$，那么
> - （1）计算犯第一类错误的概率，即在 $H_0$ 成立的前提下，落入拒绝域 $W$ 的概率
> - （2）计算犯第二类错误的概率，即在 $H_1$ 成立的前提下，落在拒绝域 $W$ 外面（即 $\overline{X} \le 3$）的概率
...
5. **显著性水平的意义**：原假设 $H_0$ 成立，经检验 $H_0$ 被拒绝的概率（可以理解为犯错的概率）【1000a.30.22】
